...
from config import order_caller, order_model, order_strategy, order_frequency, order_filter
...
alias_dict = {**order_caller,**order_model,**order_strategy,**order_frequency,**order_filter}

## metr table
metr_df = pd.read_pickle(os.path.join(DATABASE_PATH, 'rawfeat_HL_filter_metr_df.pkl'))
...
metr_df[['Caller','Frequency','Filter']] = metr_df[['software','frequency','filter']].applymap(lambda x: alias_dict[x])
metr_df_pp = metr_df.groupby(by=['depth','Caller','Frequency','Filter','patient']).mean()
metr_df_mn = metr_df.groupby(by=['depth','Caller','Frequency','Filter']).mean()
metr_df_sd = metr_df.groupby(by=['depth','Caller','Frequency','Filter']).std()
# dfs_to_sheet([metr_df, metr_df_pp, metr_df_mn, metr_df_sd], ['raw', 'per patient', 'mean', 'std'], filename='rawfeat_filter_frequency.xlsx', root_path=TAB_PATH)
...
for depth in ['30X', '50X']:
    for y_metrname in ['MCC', 'log OFO']:
        ...
        g = sns.FacetGrid(data=bar_plot_df, row='Frequency', aspect=1.6, height=4)
        g.map(
        ...
        plt.savefig(os.path.join(PLOT_PATH, 'rawfeat_filter_frequency_bar_{}-{}.pdf'.format(depth, y_metrname.replace(' ', '.'))))
        # plt.show()
            ...
            for caller in set(metr_df['Caller']):
                for filter in set(metr_df['Filter']).difference(['FNVC']):
                    ...
                    if x.shape[0] == y.shape[0]:
                        tab_list.append([frequency, caller, filter, *noparam_wrm(x,y)])
        test_tab_df_list.append(pd.DataFrame(np.array(tab_list), columns=['Frequency','Caller', 'Filter', 'FNVC Ave','FNVC SD','Filter Ave','Filter SD','statistic','pvalue']))
        test_tab_name_list.append('{}-{}'.format(depth,y_metrname))
# dfs_to_sheet(test_tab_df_list, test_tab_name_list, 'rawfeat_filter_frequency_pvalue.xlsx', TAB_PATH)
