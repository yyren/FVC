...
# dfs_to_sheet([metr_df, metr_df_pp, metr_df_mn, metr_df_sd], ['raw', 'per patient', 'mean', 'std'], filename='rawfeat_filter_mutation.xlsx', root_path=TAB_PATH)
...
for depth in ['30X', '50X']:
    ...
    plt.savefig(os.path.join(PLOT_PATH, 'rawfeat_filter_mutation_roc_scatter_{}.pdf'.format(depth)))
    # plt.show()
    ...
    g = sns.relplot(data=roc_df.loc[roc_df['depth']==depth], row='Mutation', col='Caller', kind='line', height=5, aspect=1,
        ...
        palette=[snsdeep[i] for i in [0,9,1,3,4,8]])
    g.set(xlim=(-0.1,1.1), ylim=(-0.1,1.1))
    plt.savefig(os.path.join(PLOT_PATH, 'rawfeat_filter_mutation_roc_line_{}.pdf'.format(depth)))
    # plt.show()
...
for depth in ['30X', '50X']:
    for y_metrname in ['AUC', 'G1-score', 'MCC', 'log OFO']:
        ...
        plt.savefig(os.path.join(PLOT_PATH, 'rawfeat_filter_mutation_bar_{}-{}.pdf'.format(depth, y_metrname.replace(' ', '.'))))
        # plt.show()
...
# dfs_to_sheet(test_tab_df_list, test_tab_name_list, 'rawfeat_filter_mutation_pvalue.xlsx', TAB_PATH)
