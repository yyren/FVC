...
## metr table
metr_df = pd.read_pickle(os.path.join(DATABASE_PATH, 'rawvcf_coding_filter_metr_df.pkl'))
...
# dfs_to_sheet([metr_df, metr_df_pp, metr_df_mn, metr_df_sd], ['raw', 'per patient', 'mean', 'std'], filename='rawvcf_filter_coding.xlsx', root_path=TAB_PATH)
...
for depth in ['30X', '50X']:
    for y_metrname in ['MCC', 'log OFO']:
        ...
        plt.savefig(os.path.join(PLOT_PATH, 'rawvcf_filter_coding_bar_{}-{}.pdf'.format(depth, y_metrname.replace(' ', '.'))))
        # plt.show()
...
# dfs_to_sheet(test_tab_df_list, test_tab_name_list, 'rawvcf_filter_coding_pvalue.xlsx', TAB_PATH)
