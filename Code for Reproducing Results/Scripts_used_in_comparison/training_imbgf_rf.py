...
clfkit_list, normalizer = check_loadsave(
    path.join(prespath, 'clf', 'clfkits_{}_{}_other.pkl'.format(seed, gfcode)),
    training_series, {
        'X_train': X_train,
        'y_train': y_train,
        'model_list': ['logireg', 'lsvm', 'nn', 'lgbdef'],
        'model_params': [{},{},
...
ys_df, metrs_df = check_loadsave(
    path.join(prespath, 'metr', 'metrs_{}_{}_other.pkl'.format(seed, gfcode)), 
    evaluating_series, {
        'Xs_test': Xs_test, 
        'ys_test': ys_test,
